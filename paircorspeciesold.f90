...
contains
  ...
  subroutine initPairCorrelation()
    ...
    limits=sqrt(-4.0_rk*hiuk(1)%lambda/sqrt(omega2)*log(5.0e-5_rk))
...
!       close(101)
  ...
  subroutine printPairCorrelationMPI(sgn_2)
    ...
    real(kind = rk) :: meanMPI, twoSEMMPI
...
!                // '.b' // TRIM(ADJUSTL(blockChar)) // '.dat',STATUS = 'REPLACE')
       ...
       if (myid .eq. 0) then
          !WRITE(110,'((E20.10),(E20.10),(E20.10))') &
          !     pcdata(i)%Rave/pcdata(i)%nCountsBlock/numprocs, &
          !     pcdata(i)%R2ave/pcdata(i)%nCountsBlock/numprocs, &
          !     pcdata(i)%Rave1per/pcdata(i)%nCountsBlock/numprocs
          WRITE(110,'((E20.10),(E20.10),(E20.10))') &
       ...
       do j=1,histkoko
          !Volume = 4.0_rk/3*pi*((pcdata(i)%r(j)+pcdata(i)%dr)**3-pcdata(i)%r(j)**3)
          Volume = pi*((pcdata(i)%r(j)+pcdata(i)%dr)**2-pcdata(i)%r(j)**2) ! or Area in this case          
          mean_pc = real(pcdata(i)%pcTOT(j),kind=rk)/pcdata(i)%nCountsTOT/Volume
          call MPI_Reduce(mean_pc,meanMPI,1,MPI_DOUBLE_PRECISION &
               ,MPI_SUM,0,MPI_COMM_WORLD,ierr)
          twoSEM=2.0_rk*SEM_dpc(mean_pc,i,j)
          call MPI_Reduce(twoSEM,twoSEMMPI,1,MPI_DOUBLE_PRECISION &
               ,MPI_SUM,0,MPI_COMM_WORLD,ierr)
          if (myid .eq. 0) then
             WRITE( 10, '((F15.6),(E15.6),(E15.6))') pcdata(i)%r(j)+pcdata(i)%dr/2, &
                  meanMPI/numprocs, twoSEMMPI/sqrt(real(numprocs,kind=rk))
          end if
          !WRITE( 10, '((F15.6),(E15.6),(E15.6))') pcdata(i)%r(j)+pcdata(i)%dr/2, mean_pc, 2.0_rk*SEM_dpc(mean_pc,i,j)
          !          WRITE( 101, '((F15.6),(E15.6))') pcdata(i)%r(j), real(pcdata(i)%pc(j),kind=rk)/pcdata(i)%nCountsBlock/Volume
       end do
